...
性能の数値（agent-browser の「37% トークン削減」など）は提供元ベンチが多く、独立した第三者の再現は確認できていない。導入前に自分の用途で試すのが確実だ。

セキュリティ面では、Claude in Chrome は prompt injection の管理、agent-browser はチェックサム未検証（Issue #1422）、chrome-devtools-mcp には symlink 脆弱性（CVE-2026-53765）の報告がある。組織導入では権限とシークレットの管理を前提に評価してほしい。
