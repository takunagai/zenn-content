...
この数字の読み方には注意がいります。スター数はリリース時の話題性を映す指標で、実運用での信頼とは別物です。Open Issues の生数も、スター数・公開からの月数・開発の速さに左右されます。agent-browser は 2026 年 1 月公開と新しく開発が活発なぶん Issue も積み上がりやすく、枯れて安定した Playwright MCP の 7 件と単純比較はできません。逆に Playwright MCP は公開から 15 か月、Playwright 本体のエコシステムを引き継ぐぶん実績が長い。新しさと実績のどちらを重く見るかは、あなたの本番要件しだいです。

## 各ツールの解説

各ツールで強い軸が異なります。あなたの用途に近いものを探してください。
...
- **こういうあなたに**：ログインが要る管理画面のバグを、認証し直さずに調べたい
...
- **こういうあなたに**：書いたページの表示速度・a11y・メモリを、コードと同じセッションで診断したい
...
- **こういうあなたに**：Chrome での操作を軽量・高速に回したい。Vercel / Next.js の開発フローに組み込みたい
...
- **こういうあなたに**：複数ブラウザでの E2E テストを、CI で何度でも同じ結果で回したい
...
4つを並べて分かるのは、「万能の1本」を探すこと自体が遠回りだということです。

単体での向き不向きは冒頭の表で選べます。実際に迷うのは、操作も診断も自動化も必要になる場面でしょう。そのときは1本に絞らず、操作・診断・自動化をそれぞれ得意なツールに分担させるのが、今のところ堅い選択です。

あなたの今の案件は、どのユースケースに一番近いですか。そこから逆算すれば、入れるべきツールは自然に決まります。
