...
### 既存の Chrome セッションを引き継ぐ

デフォルト設定では、MCP が呼ばれるたびに新しい Chrome ウィンドウが起動します。ログイン済みのセッションをそのまま使いたい場合や、余分なウィンドウを開かせたくない場合は、次の 2 ステップで既存の Chrome に接続できます。

**ステップ 1：Chrome をデバッグモードで起動するエイリアスを設定する**
...
追記後は `source ~/.zshrc` で反映します。以降は `chrome-debug` で Chrome を起動します。

:::message alert
Chrome は既存インスタンスが起動中の場合、後から渡したフラグを無視します。使う前に既存の Chrome ウィンドウをすべて閉じてから `chrome-debug` で起動してください。
:::

**（任意）Dock からワンクリックで起動する**

毎回ターミナルでコマンドを打ちたくない場合は、ショートカット.app でランチャーを作れます。

1. ショートカット.app を開く
2. `+` で新規ショートカット → 検索欄で「シェルスクリプトを実行」を追加
3. 以下を入力する：
...
4. 右クリック → 「名称変更」で「Chrome (debug)」などに変更
5. 右クリック → アイコンをそれっぽいもの(地球アイコンなど)に変更
6. 右クリック → 「**Dock に追加**」

![ショートカット.app でシェルスクリプトを入力する](/images/claude-code-chrome-devtools-mcp/shortcuts-shell-script.png)

![Chrome (debug) ショートカットを右クリックして Dock に追加](/images/claude-code-chrome-devtools-mcp/shortcuts-add-to-dock.png)

Dock クリックまたは Spotlight から起動できるようになります。`~/Applications/` にも自動保存されます。

**ステップ 2：MCP の接続先を固定する**
...
"mcpServers": {
  "chrome-devtools": {
    "command": "npx",
    "args": ["chrome-devtools-mcp@1.2.0", "--browser-url", "http://localhost:9222"]
  }
}
```

設定後は Claude Code を再起動します。キー名がプラグインと同じ `chrome-devtools` のため、`~/.claude.json` の設定が優先されれば二重起動しません。

:::message
**ポートのセキュリティについて**
`--remote-debugging-port=9222` で開くポートはデフォルトで `127.0.0.1`（localhost）にのみバインドされます。外部ネットワークや同一 LAN の別マシンからは接続できず、同一マシン上のプロセスからのみアクセス可能です。開発用途では許容範囲のリスクですが、パスワードや個人情報を扱うサービスにログインした状態で常時起動しておくのは避けた方が無難です。
:::
...
3. 「プロトコル モニター」にチェックを入れて DevTools を再起動
