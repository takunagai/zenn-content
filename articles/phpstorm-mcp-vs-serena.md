...
技術的には両方を同時に登録できますが、同じプロジェクトで常時併用するのはおすすめしません。理由は二つあります。

一つは、機能が大きく重なることです。シンボル検索・参照追跡・リネーム・ファイル操作は両者にあり、同じ作業を二通りのツールで行えてしまいます。PHP 案件なら PhpStorm MCP のほうが解析も精度も上なので、Serena 側は実質的に出番がなくなります。

もう一つは、ツールが増えるほどエージェントの精度が落ちることです。MCP のツール定義は呼び出しのたびにコンテキストへ読み込まれ、20 個を超えたあたりから正しいツールを選ぶ精度が下がり、30〜50 個を境に大きく崩れるという報告があります。PhpStorm MCP だけでツールは数十個あり、そこへ Serena を重ねると、重複したツールの取り違えとトークンの浪費を招きます。

指針はシンプルです ─ プロジェクト単位でどちらか一方に寄せること。PhpStorm を開いて PHP を触るなら PhpStorm MCP、エディタを問わない軽い作業や多言語のプロジェクトなら Serena。どうしても両方を登録しておきたいときは、Serena をプロジェクト（local）スコープに限定し、使わない側は無効化しておくと、ツールの氾濫を避けられます。
