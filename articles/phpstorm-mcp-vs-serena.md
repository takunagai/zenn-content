...
WordPress テーマ開発の文脈では、とくにデータベースを直接読める点と、実行時の状態をデバッガで覗ける点が調査を速くします。まずはローカルのデータベース接続を登録しておくと、「表示がおかしい」の切り分けが一段軽くなります。ここでは子テーマ中心・ビルド工程なしの WordPress テーマ開発を前提に、実際に効いた順で紹介します。
...
### 2. リモートデプロイ（FTP）─ ここは MCP に任せない

先に結論を書くと、FTP デプロイは MCP 経由ではなく、Bash + FTPS（lftp / curl）で差分ファイルを明示リストにしてアップロードするのがおすすめです。理由を順に説明します。

JetBrains の MCP サーバーには、デプロイ（FTP 転送・同期）専用のツールがありません。公開されるのは解析・DB・実行・ファイル・インスペクション・PHP デバッガ・ターミナル・VCS などのカテゴリで、PhpStorm GUI の充実したデプロイ機能（Sync with Deployed・差分ビューア・自動アップロード）は MCP には出てきません。唯一の間接経路は、汎用の `invoke_ide_action` で `PublishGroup.Upload` を発火させることだけです。`invoke_ide_action` は `filePaths` 引数で対象ファイル・フォルダを渡せるので、「テーマディレクトリを指定して Upload to Default Server」までは技術的には狙えます。

それでも MCP 経由の Upload をおすすめしないのは、次の3点です。

- **結果が読めない**: `invoke_ide_action` の戻り値は「アクションを発火した」までで、転送の成否・スキップ・エラーは IDE の File Transfer コンソール（GUI）にしか出ません。結局は `curl` などで外部から実測して確かめることになり、それなら転送そのものも自前でやるほうが一貫します
- **エラー時に手が出ない**: 競合や接続断が起きても、MCP からは介入できません
- **除外の制御が設定依存**: ディレクトリ丸ごとの転送になり、本番 webroot に置きたくないファイル（レビュー用メモ・`CLAUDE.md`・開発用の設定など）を確実に外せるかは Deployment 設定任せになります。lftp の明示リストなら 1 ファイル単位で確定的に制御でき、ログも再現性も残ります
...
なお、人間が PhpStorm GUI からデプロイするのは全く問題なく、その用途では最良のツールです。ただし Deployment のマッピングがテーマルート → webroot 全体になっていると、全体 Sync のときに `wp-config.php` やプラグインまで比較対象になります。GUI で運用するなら `Settings > Deployment > Excluded Paths` に除外を設定し、「変更を保存するたびに自動アップロード」は検証前の変更まで飛ぶので有効化しない、あたりを押さえておくと安全です。
...
一点、注意があります。JetBrains の PHP 向けスキル（後述の "PHP Project Guide" など）は、フレームワークが Laravel / Symfony のときだけその流儀に合わせ、それ以外はすべて汎用 PHP（PSR/PER）として案内します。ところが WordPress の規約は WPCS（WordPress Coding Standards）─ タブインデント・snake_case・Yoda 条件・`global $wpdb` ─ で、PSR/PER と真っ向から衝突します。WordPress 案件でそのまま従うと、かえって規約違反のコードになります。
