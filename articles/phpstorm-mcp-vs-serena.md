...
PhpStorm に登録済みの転送設定（FTP / SFTP）を使って、ファイルを本番サーバーへアップロードできます。手動で FTP クライアントを開かなくても、AI エージェントからデプロイまで完結します。
