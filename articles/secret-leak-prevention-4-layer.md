...
事後対応は順序が肝心です。**最優先はローテーション、履歴削除はそのあと**。ここを逆にすると、復旧作業をしている間もシークレットが生きたまま放置されます。

:::message
公開後、**plusone｜開発技法ノート**さんから「先にローテーション、痕跡削除はそのあと」とご指摘をいただき、この順序を修正しました。誤り箇所を指摘していただき、感謝します。
:::

**1. まず必ずパスワードをローテーション**

これが鉄則であり、最初にやることです。一度 push したシークレットは「漏れたもの」として扱う。履歴を消しても、その間にクローンした人、フォーク、GitHub の内部キャッシュ、検索インデックス、どこに残っているか分かりません。

なぜ削除より先かというと、履歴の書き換えは時間がかかるうえ失敗もしうる作業で、やっている間もシークレットは有効なままだからです。先に値を切り替えてしまえば、クリーンアップにどれだけ手間取っても、漏れたシークレットはただの無効な文字列になる。GitHub 公式も「機密データがシークレットなら、**最初のステップ**として revoke / rotate せよ。それで問題が解決することも多い」と明言しています。

履歴から消すのは「これ以上の露出を防ぐ」ためであって、「無かったことにする」ためではない。**該当する API キー・パスワードは全部新しい値に切り替えてください**。これをやらないと後続の作業の意味が半分以下になります。

**2. 履歴から削除**

ローテーションで封じ込めたうえで、履歴も掃除します。`git filter-branch` は非推奨。今は [git filter-repo](https://github.com/newren/git-filter-repo) または [BFG Repo-Cleaner](https://rtyley.github.io/bfg-repo-cleaner/) を使います。
