...
## AIエージェントに実行させるプロンプトレシピ

初めて使用する際は、許可ダイアログを押さえる必要があります。  
確実に gog を使用させたい場合は `gog スキルを使って` と指示してください。  
Googleサービスを使うためのプロジェクトを作成しておき、カスタム指示に仕込んでおくのもいいかも。
...
`gog` とそれを扱うエージェントスキルを導入することで、会話で Google サービスを操作できるようになりました。
